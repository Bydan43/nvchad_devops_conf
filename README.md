...
Конфиг использует упрощённый профиль (`preset = classic`): показываются только маппинги с `desc`, служебные пресеты отключены, а часть автогенерируемых англоязычных подсказок локализуется через `replace.desc`.
...
- Явный вызов шпаргалки:
  - `Пробел` `h` `h` — подсказки с учётом текущего буфера (`global = false`);
  - `Пробел` `h` `H` — все доступные сочетания (`global = true`);
  - `Пробел` `h` `l` — показать меню только по `<leader>`;
  - `Пробел` `h` `k` — открыть `NvCheatsheet`.

Совместимость:
- `Пробел` `?` и `Пробел` `!` оставлены как старые алиасы.
...
| Клавиши | Режим | Действие |
|---------|-------|----------|
| `<leader> e` | `n` | Показать/скрыть боковое дерево файлов. |
| `<leader> o` | `n` | Перевести фокус в дерево файлов. |
